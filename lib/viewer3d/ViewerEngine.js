...
export default class ViewerEngine {
  constructor(container) {
    ...
    // Exposición baja a propósito: con valores cercanos a 1 el modelo (paredes
    // blancas) queda en el hombro de la curva ACES, donde todo se aplasta
    // contra el blanco y se pierde el modelado del volumen.
    ...
    // El zoom se dirige al punto bajo el cursor, como en un visor CAD
    ...
    // El render es bajo demanda para no consumir GPU con la escena quieta.
    // Mientras el usuario interactúa se mantiene una ventana de actividad:
    // así el zoom con rueda o pellizco nunca se queda a la espera de que
    // otro evento dispare el siguiente fotograma.
    this.activeUntil = 0;
    this.onControlsChange = () => this.keepAlive();
    this.controls.addEventListener('change', this.onControlsChange);

    this.onPointerActivity = () => this.keepAlive();
    ['wheel', 'pointerdown', 'pointermove', 'pointerup', 'touchstart', 'touchmove'].forEach(
      (type) =>
        this.renderer.domElement.addEventListener(type, this.onPointerActivity, {
          passive: true,
        })
    );
    ...
    // El entorno da una luz uniforme desde todas las direcciones: aporta color
    // y reflejos, pero no diferencia unas caras de otras. El peso principal lo
    // lleva la luz direccional, que es la que da volumen a la arquitectura.
    this.keyLight = new THREE.DirectionalLight(0xffffff, 2.5);
    this.keyLight.position.set(4, 8, 6);
    this.scene.add(this.keyLight);

    this.fillLight = new THREE.HemisphereLight(0xffffff, 0x35383d, 0.18);
  ...
  _whiteMaterial(side) {
    ...
    if (!this._whiteCache[side]) {
      this._whiteCache[side] = new THREE.MeshStandardMaterial({
        // Blanco de maqueta, no blanco puro: deja margen tonal para que se
        // distingan las caras en lugar de fundirse en una silueta plana.
        color: 0xdcd9d3,
        roughness: 0.92,
        metalness: 0,
        envMapIntensity: 0.8,
  ...
  _wireMaterial() {
    if (!this._wireCache) {
      this._wireCache = new THREE.MeshBasicMaterial({
        ...
        transparent: true,
        opacity: 0.45,
        // Sin mapeo tonal: son líneas de dibujo, no superficies iluminadas
        toneMapped: false,
      });
    }
  ...
  /** Mantiene el render activo un instante más (interacción en curso). */
  keepAlive(ms = 700) {
    this.dirty = true;
  ...
  _tick() {
    ...
    let dirty = this.dirty || performance.now() < this.activeUntil;
  ...
  dispose() {
    ...
    ['wheel', 'pointerdown', 'pointermove', 'pointerup', 'touchstart', 'touchmove'].forEach(
      (type) => this.renderer.domElement.removeEventListener(type, this.onPointerActivity)
    );
