import { ceo, site } from '@/data/site';
...
export const PERSON_ID = `${site.url}/#carlos-lasso`;
...
export function organizationSchema() {
  return {
    ...
    sameAs: [site.instagram, site.facebook, site.linkedin],
...
/**
 * El profesional que responde por el trabajo.
 *
 * `hasCredential` lleva las matrículas con la entidad que las expide, que es
 * lo que permite a cualquiera —persona o sistema— verificarlas por su cuenta.
 * No se publica el documento de identidad en ninguna propiedad.
 */
export function personSchema() {
  return {
    ...
    knowsAbout: site.knowsAbout,
    alumniOf: [
      ...new Set(ceo.titulos.map((t) => t.entidad)),
    ...
    hasCredential: [
      ...ceo.titulos.map((t) => ({
        ...
        name: t.titulo,
        educationalLevel: t.titulo.startsWith('Especialista') ? 'Postgrado' : 'Pregrado',
        recognizedBy: { '@type': 'CollegeOrUniversity', name: t.entidad },
      })),
      ...ceo.credenciales.map((c) => ({
        '@type': 'EducationalOccupationalCredential',
        credentialCategory: 'professional license',
        name: `${c.nombre} n.º ${c.numero}`,
        identifier: c.numero,
        recognizedBy: { '@type': 'Organization', name: c.entidad, url: c.url },
      })),
    ],
  };
}
...
export function webPageSchema({ path, name, description, breadcrumb, image, author }) {
  return {
    '@type': 'WebPage',
    ...
    about: { '@id': ORG_ID },
    publisher: { '@id': ORG_ID },
    // El contenido normativo lo firma un arquitecto con matrícula vigente,
    // no una web anónima. Es la diferencia entre un texto y una fuente.
    ...(author ? { author: { '@id': PERSON_ID } } : {}),
