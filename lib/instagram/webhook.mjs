 ...
 *  - Una firma que no valida SIEMPRE produce 403, con diagnóstico o sin él.
 *
 * Diagnóstico temporal (`INSTAGRAM_WEBHOOK_DEBUG`): registra por qué falla la
 * validación de la firma —presencia, longitud y huella irreversible de cada
 * candidato a App Secret— sin revelar ningún valor. Debe retirarse una vez
 * identificada la clave correcta.
 */
...
/**
 * Nombres de las variables de entorno que pueden contener el App Secret con el
 * que Meta firma los eventos. Se prueban en orden.
 *
 * Existen dos claves distintas y NO intercambiables en el panel de Meta:
 *   - Configuración → Básica → "Clave secreta de la app"        (app de Meta)
 *   - Instagram → Configuración de la API → "Clave secreta de
 *     la app de Instagram"                                      (app de Instagram)
 *
 * `INSTAGRAM_APP_SECRET_ALT` existe para poder tener las dos cargadas a la vez
 * y que el diagnóstico determine cuál produce el HMAC correcto, en lugar de ir
 * cambiando el valor a ciegas.
 */
export const SECRET_CANDIDATE_NAMES = ['INSTAGRAM_APP_SECRET', 'INSTAGRAM_APP_SECRET_ALT'];
...
/**
 * Huella IRREVERSIBLE de un secreto: SHA-256 truncado a 12 caracteres hex.
 *
 * Sirve para comparar «¿es este el mismo valor que aquel?» sin revelar nada.
 * No permite recuperar el secreto ni por fuerza bruta (el espacio de entrada de
 * un App Secret son 128 bits).
 */
export async function secretFingerprint(value) {
...
/**
 * Prueba la firma recibida contra TODOS los candidatos a App Secret y devuelve
 * un informe apto para registrar.
 *
 * Para cada candidato se prueba el valor tal cual y, si difiere, su versión
 * recortada con `trim()`: pegar un secreto en un panel web arrastra con
 * frecuencia un espacio o un salto de línea invisible que rompe el HMAC por
 * completo.
 *
 * El informe NUNCA contiene el valor del secreto, solo metadatos y una huella
 * irreversible.
 *
 * @returns {{ matched: {name: string, usedTrim: boolean} | null, secretos: object[] }}
 */
export async function diagnoseSignature(rawBytes, headerValue, env) {
  const received = typeof headerValue === 'string' ? headerValue.trim() : null;
  const secretos = [];
  let matched = null;

  for (const name of SECRET_CANDIDATE_NAMES) {
    const value = readEnv(env, name);

    if (typeof value !== 'string' || value.length === 0) {
      secretos.push({ nombre: name, presente: false });
      continue;
    }
    ...
    const entry = {
      ...
      longitudTrim: trimmed.length,
      espaciosSobrantes: trimmed.length !== value.length,
      pareceHex32: /^[0-9a-f]{32}$/i.test(trimmed),
      huella: await secretFingerprint(value),
      coincide: false,
      coincideTrim: false,
    };

    if (received) {
      entry.coincide = safeCompare(await computeSignature(rawBytes, value), received);
      entry.coincideTrim = entry.espaciosSobrantes
        ? safeCompare(await computeSignature(rawBytes, trimmed), received)
        : entry.coincide;
    }

    secretos.push(entry);

    if (!matched && (entry.coincide || entry.coincideTrim)) {
      matched = { name, usedTrim: !entry.coincide };
    }
  }

  return { matched, secretos };
}
...
export async function handleEvent(request, env, logger = console) {
  const debug = Boolean(readEnv(env, 'INSTAGRAM_WEBHOOK_DEBUG'));
  ...
  const { matched, secretos } = await diagnoseSignature(rawBytes, signature, env);
  const hayCandidatos = secretos.some((s) => s.presente);

  if (debug) {
    // Diagnóstico temporal. Solo metadatos: ningún secreto, ningún token,
    // ningún contenido de mensaje.
    logger.log('[instagram-webhook][diag]', {
      secretos,
      cabeceras: {
        'x-hub-signature-256': signature ? `presente (${signature.length} ch)` : 'AUSENTE',
        'x-hub-signature': request.headers.get('x-hub-signature') ? 'presente' : 'AUSENTE',
      },
      rawBodyBytes: rawBytes.byteLength,
      contentLengthDeclarado: request.headers.get('content-length'),
      resultado: matched
        ? `VÁLIDA con ${matched.name}${matched.usedTrim ? ' (tras trim)' : ''}`
        : 'NINGÚN CANDIDATO COINCIDE',
    });
  }
